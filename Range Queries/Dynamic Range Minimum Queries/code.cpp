...
ll sum(ll node,ll ml, ll mr,ll l, ll r, const vl& v){
    ...
    return min(sum(2*node,ml,last_in_left,l,r,v), sum(2*node  + 1,last_in_left + 1, mr,l,r,v));
...
int main()
{
    ...
    while(q--){
        ...
        case 1:
            ...
            for(int j = (N+a)/2 ; j >= 1; j/=2){
                v[j] = min(v[2*j],v[2*j+1]);
            }
            // debug(v);
